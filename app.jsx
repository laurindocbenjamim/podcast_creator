...
const App = () => {
    ...
    const [backgroundOpacity, setBackgroundOpacity] = useState(1); // NEW: Opacity for the background color (0 to 1)
    ...
    const drawWaveform = useCallback(() => {
        ...
        // Draw background image if loaded, otherwise draw background color with opacity
        if (loadedBgImageRef.current) {
            ctx.drawImage(loadedBgImageRef.current, 0, 0, width, height);
        } else {
            ctx.fillStyle = hexToRgba(backgroundColor, backgroundOpacity); // Use RGBA for background color
    ...
    }, [isPlaying, waveformStyle, waveformColor, backgroundColor, backgroundOpacity, textOverlay]); // Added backgroundOpacity to dependencies
    ...
    const drawWaveformVisuals = (ctx, width, height, analyser, style, color, text) => {
        ...
        switch (style) {
            ...
            case 'circles':
                ...
                break;
            case 'frequency-bars':
                const freqBarWidth = width / analyser.frequencyBinCount;
                for (let i = 0; i < analyser.frequencyBinCount; i++) {
                    const barHeight = dataArray[i] * 2; // Scale for frequency data
                    ctx.fillRect(i * freqBarWidth, height - barHeight, freqBarWidth * 0.8, barHeight);
                }
                break;
            case 'smooth-lines':
                ctx.beginPath();
                ctx.lineJoin = 'round'; // Make lines smooth
                ctx.lineCap = 'round'; // Make line caps round
    ...
    }, [waveformStyle, waveformColor, backgroundColor, backgroundOpacity, backgroundImage, textOverlay, drawWaveform]); // Added backgroundOpacity to dependencies
        ...
        <div className="min-h-screen bg-[var(--clr-dark-a0)] flex flex-col items-center py-8 font-sans">
            <div className="w-full max-w-4xl bg-[var(--clr-surface-a0)] p-8 rounded-lg shadow-xl">
                ...
                <div className="mb-6 p-4 border border-[var(--clr-surface-a10)] rounded-lg bg-[var(--clr-surface-a10)] grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label htmlFor="background-opacity" className="block text-lg font-medium text-[var(--clr-light-a0)] mb-2">
                            Background Opacity
                        </label>
                        <input
                            type="range"
                            ...
                            className="mt-1 block w-full h-10 rounded-md accent-[var(--clr-primary-a0)]"
                        />
